...
export default function AnglerQuestionnaire({ onComplete }) {
  ...
  const handleSelect = (v) => {
    if (current.key === 'gear') {
      if (v === 'none') {
        ...
        setAnswers({ ...answers, gear: value.includes('none') ? [] : ['none'] });
      } else {
        // Selecting any real gear deselects None
        const without_none = value.filter(x => x !== 'none');
        const arr = without_none.includes(v)
          ? without_none.filter(x => x !== v)
          : [...without_none, v];
        setAnswers({ ...answers, gear: arr });
      }
      return;
    }
  ...
  // ── Priorities screen (shown after all 7 questions) ──────────────────────
  if (showPriorities) {
    return (
      <div className="cw-screen cw-intake">
        ...
        <div className="cw-intake-actions">
          ...
          <button
            className="cw-btn cw-btn-primary cw-btn-grow"
            disabled={priorities.length < 3}
            onClick={finishWithPriorities}
          >
